...
// Firestore API URL
const firestoreProductFetchUrl = "https://firestore.googleapis.com/v1/projects/online-shopping-fd20d/databases/(default)/documents/products";
...
async function fetchProducts() {
    try {
        ...
        allProducts = data.documents.map(doc => {
            return {
                id: doc.name.split('/').pop(), // Extract the document ID from the Firestore document name
...
function displayProducts() {
    ...
    productsToDisplay.forEach(product => {
        ...
        // Add a click event listener to the row for redirection
        row.addEventListener('click', () => {
            window.location.href = `updateForm.html?id=${product.id}&title=${encodeURIComponent(product.title)}&description=${encodeURIComponent(product.description)}&quantity=${product.quantity}&price=${product.price}&cat_id=${encodeURIComponent(product.cat_id)}`;
        });
